...
const CountrySelector = ({
  ...
  return __jsx(react__WEBPACK_IMPORTED_MODULE_0___default.a.Fragment, null, __jsx(semantic_ui_react__WEBPACK_IMPORTED_MODULE_2__["Dropdown"], {
    ...
    onChange: (e, data) => {
      ...
      onChange(data.value, countries.countries.find(x => x.iso2 === data.value).iso3);
    },
    __source: {
      fileName: _jsxFileName,
      lineNumber: 37
...
const StateStats = ({
  ...
  states = states.filter(x => x.lastUpdate === states[0].lastUpdate); //console.log("State wise info 2", states);
  ...
  const onChange = selectedValue => {
    let selected = states.find(state => state.combinedKey === selectedValue); // console.log("Selected state info", selected);
...
const Index = () => {
  ...
  const onChange = (iso2, iso3) => {
    console.log("index js onchange", iso2, iso3); //this is to load state dropdown for StateStats
...
const getChartData = (chartType, apiData) => {
  let sliced = apiData.slice(-40); //console.log("Api data", sliced);
